...
}if ( ! function_exists("checkedOrNot")) {
    
    function checkedOrNot(bool $value, string $colorClass = 'text-success', string $showThisIfNo = ''): string
    {
        $checkIcon = config('Basics')->theme['name'] == 'Bootstrap5' ? 'bi bi-check-lg' : 'fas fa-check';
        $item = '<i class="'.$colorClass.' '.$checkIcon.'"></i>';
        return $value ? $item : $showThisIfNo;
    }
