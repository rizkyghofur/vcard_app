...
<?= $this->section('additionalExternalJs') ?>
    ...
    <script src="https://cdn.datatables.net/1.12.1/js/dataTables.<?=config('Basics')->theme['name'] == 'Bootstrap5' || config('Basics')->theme['name'] == 'AdminLTE4' ? 'bootstrap5' : 'bootstrap4' ?>.min.js"></script>
<?= $this->endSection() ?>
    ...
    <?php if (isset($usingServerSideDataTable) && $usingServerSideDataTable) : ?>
        ...
        <?php if (isset($usingClientSideDataTable) && $usingClientSideDataTable) { ?>
            ...
            theTable = $('.using-data-table').DataTable({
                ...
                "stateSave": true,
                "language": {
                    url: "/assets/dt/<?= config('Basics')->languages[$currentLocale] ?? ( $currentLocale ?? config('Basics')->i18n ) ?>.json"
                },
                "columnDefs": [
                    {
                        orderable: false,
